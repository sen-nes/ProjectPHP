...
<?php
   if($_GET){
       if ($_GET["id_department"]) {
        ...
        $sql = "select name from departments where id = $id_department";
        $depName = mysqli_fetch_assoc(mysqli_query($conn, $sql));
         if(! $depName ) {
   ...
   if($_POST) {
       if ($_POST["name_department"]) {
          $name_department = $_POST['name_department'];
        $sql = "UPDATE departments ". "SET name='$name_department'".
                    ...
                    <form action = "<?php $_PHP_SELF ?>" method = "POST">
                        <div class="form-horizontal centermeh">
                            <div class="form-group">
                                <label class="control-label col-md-2" for="name_department">Name</label>
                                <div class="col-md-10">
                                    <?php echo "<input id=\"name_department\" class=\"form-control\" type = \"text\" name = \"name_department\" value = \"$depName[name]\"/>";?>
                                </div>
                            </div>
